...
# solana-block-tracker

This module walks the Solana blockchain, keeping track of the latest block.
It uses a SolanaQuery provider as a data source and will continuously poll for the next block.

```js
const SolanaQuery = require('solana-query');
const PollingBlockTracker = require('solana-block-tracker');

const provider = new SolanaQuery('https://api.mainnet-beta.solana.com');

...
class SubscribeBlockTracker extends BaseBlockTracker {
  ...
  async _start() {
    if (this._subscriptionId == null) {
      try {
        let blockNumber = (await this._call('getEpochInfo')).absoluteSlot;
        this._subscriptionId = await this._call('slotSubscribe');
  ...
  async _end() {
    if (this._subscriptionId != null) {
      try {
        await this._call('slotUnsubscribe', this._subscriptionId)
  ...
  _handleSubData(err, data) {
    if (data.method === 'slotSubscribe' && data.params.subscription === this._subscriptionId) {
